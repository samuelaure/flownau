...
import { fetchApprovedRecord, updateRecordToProcessed } from './core/airtable';
...
const run = async () => {
  ...
  logger.info({ templateId: activeConfig.id }, '🚀 Astromatic: Starting automation cycle...');
  ...
  try {
    ...
    if (!payload) {
      logger.info({ templateId: activeConfig.id }, 'Nothing to process. Exiting.');
